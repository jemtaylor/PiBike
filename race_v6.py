...
import math
import sys
from bike_v6 import Bike
from route_v6 import Route
...
def plotX(groundpos):
   #print (groundpos, groundpos*40)
   return groundpos*40


def plotBike(screen, bike, leftedge, bikepic, route):
 
   dist = bike.GetDistance()
   height = route.GetHeight(int(dist*1000))
   bikeX = plotX((dist-leftedge) * 1000)
   bikeY = 300 - height/4  + bike.GetId()*3
   #print (dist, height, bikeY)

   rotated_bike = pygame.transform.rotate(bikepic, bike.GetRotation() )
...
def run(screen, playerBike, computerBikes, Race_Length):
   ...
   while running:
      ...
      # if the computer falls more than 15m behind the player then he will fall off the screen:
      if leftedge < player_dist - 0.015:
         leftedge = player_dist - 0.015 

      left = int(leftedge * 1000)
      groundpos =left-10
      routelen = route.GetLength()

      while groundpos < left + 50:
         ...
         if item == "tree":
            screen.blit(tree, (plotX(groundpos-left),300-(groundheight/4)-50)) 

         if item == "cafe":
            screen.blit(cafe, (plotX(groundpos-left),300-(groundheight/4)-70)) 

         if item == "bookshop":
            screen.blit(bookshop, (plotX(groundpos-left),300-(groundheight/4)-70)) 

         if item == "giftshop":
            screen.blit(giftshop, (plotX(groundpos-left),300-(groundheight/4)-70)) 
         
         if item == "finish":
            screen.blit(finish, (plotX(groundpos-left),300-(groundheight/4)-120)) 
         
         if item == "flamerouge":
            screen.blit(flamerouge, (plotX(groundpos-left),300-(groundheight/4)-120)) 
         
         if item == "crowd":
            screen.blit(crowd, (plotX(groundpos-left),300-(groundheight/4)-60)) 
         
         groundpos = groundpos + 1

      groundpos =left
      while groundpos < left + 20:
         
         groundheight = route.GetHeight(groundpos)

         # Draw the ground green with a red line at the finish
         if groundpos == route.GetLength():
            pygame.draw.rect(screen, red, (plotX(groundpos-left), 300-(groundheight/4)+50, 40, 20)) 
         else:
            pygame.draw.rect(screen, green, (plotX(groundpos-left), 300-(groundheight/4)+50, 40, 20)) 
        
         groundpos = groundpos + 1

      for b in computerBikes:
         plotBike(screen, b, leftedge, bluebike, route)
         #pygame.draw.rect(screen, red, (computerbikeX, computerbikeY, 5, 5))

      if playerBike.GetFinished()==True: 
         plotBike(screen, playerBike, leftedge, greybike, route)
      else:
         plotBike(screen, playerBike, leftedge, redbike, route)
